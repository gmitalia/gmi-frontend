...
export default function PageResults(props) {
  ...
  useEffect(
    function fetchResults() {
      GMIApi.getInstance().getResults(query.contest, (data) => {
        if (!data.success) setError(data.error || "invalid results");
  ...
  const leaderboard = results.games_data
    ? results.games_data.map((game, index) => (
        <tr
          key={game.id}
  ...
  const judges_report = results.judges_data
    ? results.judges_data.map((judge, index) => (
