...
export default function VoteForm(props) {
    ...
    function onSave() {
        ...
        if (formValidation(data) == false) return null;
        props.onChange(data,
            props.index,
            (success) => setSaved(success))
    }

    let prosOptions = prosList.map((elem, i) => <option key={i} value={i} >{elem}</option>)
    ...
    let consOptions = consList.map((elem, i) => <option key={i} value={i} >{elem}</option>)
        ...
        <div >
            ...
            <div className="my-3 row">
                <div>
                    ...
                    <span class="ml-3" style={{ color: "green" }}>{saved && "Salvato"}</span>
                    <span class="ml-3" style={{ color: "red" }}>{formError}</span>
                </div>
            </div>
        </div >
    )
